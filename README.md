...
Like any JavaScript object, the state can be a deeply nested tree of objects. We refer to nested objects in the state as partial state. A single state tree does not conflict with modularity — see [Nested Actions] to find out how to update nested objects and split your state and actions.
...
Actions can be nested inside namespaces. Beware the Jabberwock, my son!
...
To help you create Virtual Nodes in a more compact way, Hyperapp provides a hyperscript-style <samp>h</samp> function. The <samp>h</samp> function takes a [Component](#components) or element's name, optional attributes and optional array of children elements. A component is a pure function that returns a Virtual Node.
...
Components receive children elements via the second argument.
...
This event is fired before the element is removed from the DOM. Use it to create slide/fade out animations. Call <samp>done</samp> inside the function to remove the element.
